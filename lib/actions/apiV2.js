...
import LtpPlanQuery from './ltpPlanQuery'
...
export function routingQuery(searchId = null, updateSearchInReducer) {
  ...
  return function (dispatch, getState) {
    ...
    console.debug('currentQuery: ' + JSON.stringify(currentQuery))
    ...
    console.debug('activeModes: ' + JSON.stringify(activeModes))
    ...
    // Note : patched so that car mode is not filtered out when generating combinations
    const hasCar = activeModes.find((m) => m.mode === 'CAR')
    const hasBicycle = activeModes.find((m) => m.mode === 'BICYCLE')
    const hasTransit = activeModes.find((m) => m.mode === 'TRANSIT')
    let combinations
    if (hasCar) {
      if (activeModes.length === 1) {
        combinations = [baseQuery]
      } else if (hasBicycle && !hasTransit) {
        combinations = [
          {
            ...baseQuery,
            ...{ modes: [activeModes.find((m) => m.mode === 'BICYCLE')] }
          },
          {
            ...baseQuery,
            ...{ modes: [activeModes.find((m) => m.mode === 'CAR')] }
          }
        ]
      }
    }
    combinations ||= modes ? [baseQuery] : generateCombinations(baseQuery)
    ...
    combinations.forEach((combo, index) => {
      console.debug('LtpPlanQuery: ' + Object.keys(LtpPlanQuery))
      const query = generateOtp2Query(combo, LtpPlanQuery)
      dispatch(
        createGraphQLQueryAction(
          query.query,
          {
            ...query.variables,
            ...{ numItineraries: 10, searchWindow: 36000 }
          },
          (response) => {
